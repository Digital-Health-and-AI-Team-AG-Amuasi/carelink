...
@section('content')
    <div class="container mx-auto p-6 bg-white rounded-lg shadow-xl max-w-4xl">
        <h2 class="text-3xl font-bold text-gray-800 mb-8 text-center">Create New Patient Record</h2>
        ...
        @endif
            ...
            <form method="post" action="{{route('patients.store')}}" class="space-y-6" id="patientForm">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="tel" id="phone" name="phone" value="{{ old('phone') }}"
                               class="input-field w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                               placeholder="e.g., +23300000000"/>
                    </div>
                    <div>
                        <label for="lhims_number" class="block text-sm font-medium text-gray-700 mb-1">LHIMS
                            Number</label>
                        <input type="text" id="lhims_number" name="lhims_number" value="{{ old('lhims_number') }}"
                               class="input-field w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
...
{{--                                        @if(old('gender') == $gender->value) selected @endif>{{ $gender->value }}</option>--}}
{{--                            @endforeach--}}
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select id="marital_status" name="marital_status"
                            ...
                            @foreach(MaritalStatus::cases() as $marital_status)
                                <option value="{{ $marital_status->value }}"
                                        @if(old('$marital_status') == $marital_status->value) selected @endif>
                    ...
                    <div>
                        ...
                        <select id="nhis_status" name="nhis_status"
                                class="select-field w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            <option value="">Select Status</option>
                            <option value="Active" @if(old('nhis_status') == 'Active') selected @endif>Active</option>
                            <option value="Inactive" @if(old('nhis_status') == 'Inactive') selected @endif>Inactive
                            </option>
                            <option value="Not Applicable" @if(old('nhis_status') == 'Not Applicable') selected @endif>
                                Not Applicable
                            </option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" id="ega" name="ega" value="{{ old('ega') }}"
                               class="input-field w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                               placeholder="Expected Gestational Age"/>
                    </div>
                </div>
            </div>
            ...
            <!-- Social & Other Information Section -->
            <div class="bg-gray-50 p-6 rounded-lg shadow-inner">
                <h3 class="text-xl font-semibold text-gray-700 mb-4 border-b pb-2">Social & Other Information</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">

                    <!-- Social History using Tagify -->
                    <div class="md:col-span-2">
                        <label for="social_history" class="block text-sm font-medium text-gray-700 mb-1">Social History</label>
                    ...
                    <!-- Medical Records Notes using Tagify -->
                    <div class="md:col-span-2">
                        <label for="medical_records" class="block text-sm font-medium text-gray-700 mb-1">Medical Records Notes</label>
                        <input id="medical_records" name="medical_records"
                               placeholder="Diabetes file; MRI scan; Prescriptions; Blood test"
                               class="tagify-field w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"/>
                    </div>

                    <!-- Current Medication using Tagify -->
                    <div class="md:col-span-2">
                        <label for="current_medication" class="block text-sm font-medium text-gray-700 mb-1">Current Medication</label>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="notes" name="notes"
                                  ...
                                  rows="5" placeholder="Any other relevant notes..."></textarea>
                    </div>
                </div>
            </div>
...
@push("scripts")
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Add initial medication entry if old data exists
            {{--const oldMedications = "{{ old('current_medication') }}";--}}
            {{--if (oldMedications) {--}}
            {{--    try {--}}
            {{--        const parsedMedications = JSON.parse(oldMedications.replace(/&quot;/g, '"')); // Handle HTML entities--}}
            {{--        parsedMedications.forEach(med => createMedicationEntry(med));--}}
            {{--        updateMedicationsDisplay(); // Display initial medications--}}
            {{--    } catch (e) {--}}
            {{--        console.error("Error parsing old medications data:", e);--}}
            {{--    }--}}
            {{--} else {--}}
            {{--    // Add one empty medication entry by default if no old data--}}
            ...
            if(addVitalBtn)
                addVitalBtn.addEventListener('click', function () { createVitalEntry(); });

            // Event listener for "Display Vitals" button
            if(displayVitalsBtn)
                displayVitalsBtn.addEventListener('click', function () {updateVitalsDisplay();});

            // Event listener for "Add Medication" button
            if(addMedicationBtn)
                addMedicationBtn.addEventListener('click', function () {createMedicationEntry();});

            // Event listener for "Display Medications" button
            if(displayMedicationsBtn)
                displayMedicationsBtn.addEventListener('click', function () {updateMedicationsDisplay();});
